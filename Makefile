...
FEATURES =?
FINAL_CMD =? mv
BUILD_CMD =? cargo build --features $(FEATURES) --target $(TARGET)
IMAGE_CMD =? cargo build --features $(FEATURES) --target $(TARGET)
IMAGE_NAME =? FOMOSv3.img

ifeq ($(ARCH), x86_64)
	TARGET = x86_64-FOMOS.json
	BUILD_CMD = cargo build --features $(FEATURES) --target $(TARGET)
	IMAGE_CMD = cargo bootimage --features $(FEATURES) --target $(TARGET)
	IMAGE_NAME = FOMOSv3.img
...
ifeq ($(ARCH), aarch64)
	...
	BUILD_CMD = cargo build --features $(FEATURES) --target $(TARGET) --release
